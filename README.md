...
The standalone science sidecar fails closed unless `CORTEXLUME_TOKEN` is set.
For an intentionally local development-only run without a generated token,
set `CORTEXLUME_ALLOW_INSECURE_DEV_AUTH=1`; it uses the well-known development
token and prints a prominent warning. Packaged Electron and MCP launches always
inject a random token and do not use this development switch.
